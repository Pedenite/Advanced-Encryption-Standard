...
class Decipher():
    def __init__(self, msg, key, rounds):
        ...
        for x in range(rounds):
            self.expandKey(x, self.s_box_inv)
        ...
        for i in range(rounds):
            ...
            self.shiftRows()
            self.subBytes()
            self.expandKey(rounds-1-i, self.s_box)
    ...
    def shiftRows(self):
        ...
        for i in range(len(blocks)):
            block = convert_matrix(blocks[i])
            for x in range(1, len(block)):
                block[x] = block[x][4-x:] + block[x][:4-x]

            blocks[i] = convert_list(block)
    ...
    def expandKey(self, round, s_box):
        for i in range(len(self.key)):
            self.key[i] ^= s_box[self.key[i]] ^ (rcon(round) if i == 0 else 0)
